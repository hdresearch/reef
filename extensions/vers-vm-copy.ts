...
class VersSSHClient {
  ...
  async scpArgs(vmId: string): Promise<string[]> {
    ...
    return [
      ...
      "ConnectTimeout=30",
      "-o",
      `ProxyCommand=openssl s_client -connect ${hostname}:443 -servername ${hostname} -quiet 2>/dev/null`,
    ];
  }
    ...
    async execute(_id, params) {
      ...
      if (srcParsed.vmId && !dstParsed.vmId) {
        ...
        if (isDir) {
          ...
          if (files.length === 0) {
            return {
              content: [{ type: "text", text: `No files found at ${srcParsed.path} on VM ${vmId.slice(0, 12)}` }],
              details: {},
            };
          }
          let copied = 0;
          ...
          return {
            content: [
              {
                type: "text",
                text: `Copied ${copied} file(s) from VM ${vmId.slice(0, 12)}:${srcParsed.path} → ${localPath}`,
              },
            ],
          ...
          return {
            content: [
              {
                type: "text",
                text: `Copied VM ${vmId.slice(0, 12)}:${srcParsed.path} → ${localPath} (${formatSize(content.length)})`,
              },
            ],
        ...
        if (isDir) {
          ...
          return {
            content: [
              {
                type: "text",
                text: `Copied ${copied} file(s) from ${localPath} → VM ${vmId.slice(0, 12)}:${dstParsed.path}`,
              },
            ],
          ...
          return {
            content: [
              {
                type: "text",
                text: `Copied ${localPath} → VM ${vmId.slice(0, 12)}:${dstParsed.path} (${formatSize(content.length)})`,
              },
            ],
        ...
        if (isDir) {
          ...
          return {
            content: [
              {
                type: "text",
                text: `Copied ${copied} file(s) from VM ${srcVmId.slice(0, 12)}:${srcParsed.path} → VM ${dstVmId.slice(0, 12)}:${dstParsed.path}`,
              },
            ],
          ...
          return {
            content: [
              {
                type: "text",
                text: `Copied VM ${srcVmId.slice(0, 12)}:${srcParsed.path} → VM ${dstVmId.slice(0, 12)}:${dstParsed.path} (${formatSize(content.length)})`,
              },
            ],
...
async function resolveVmId(_client: VersSSHClient, partialId: string): Promise<string> {
