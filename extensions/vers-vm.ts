...
import { access, writeFile as fsWriteFile, mkdir, readFile, writeFile } from "node:fs/promises";
...
interface NewVmResponse {
  vm_id: string;
}
interface VmDeleteResponse {
  vm_id: string;
}
interface VmCommitResponse {
  commit_id: string;
}
interface VmSSHKeyResponse {
  ssh_port: number;
...
function loadVersKeyFromDisk(): string {
  ...
  try {
    ...
    const data = require("node:fs").readFileSync(keysPath, "utf-8");
  ...
  try {
    ...
    const data = require("node:fs").readFileSync(configPath, "utf-8");
    const parsed = JSON.parse(data);
...
class VersClient {
  ...
  async branch(vmId: string): Promise<NewVmResponse> {
    const raw = await this.request<NewVmResponse | { vms: NewVmResponse[] }>(
      "POST",
  ...
  async openMaster(vmId: string): Promise<void> {
    ...
    if (this.masterActive.has(vmId)) {
      const alive = await new Promise<boolean>((resolve) => {
        ...
        execFile(
          "ssh",
          ["-o", `ControlPath=${args}`, "-O", "check", `root@${vmId}.vm.vers.sh`],
          { timeout: 3000 },
          (err) => {
            resolve(!err);
          },
  ...
  async exec(
    ...
    const controlArgs = this.masterActive.has(vmId) ? ["-o", "ControlMaster=no"] : ["-o", "ControlMaster=no"];
    return new Promise((resolve, reject) => {
      execFile(
        "ssh",
        [...controlArgs, ...args, command],
  ...
  execStreaming(
    ...
    // biome-ignore lint/suspicious/noAsyncPromiseExecutor: SSH command needs async setup inside promise for timeout handling
...
export default function versVmExtension(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(_id, params) {
      ...
      return {
        content: [{ type: "text", text: `Restored from commit ${commitId} -> VM ${result.vm_id}` }],
        details: result,
      };
    },
  });
  ...
  pi.registerTool({
    ...
    label: "Use Vers VM",
    description:
      "Set the active VM. After calling this, read/bash/edit/write tools execute on the VM via SSH instead of locally.",
    parameters: Type.Object({
    ...
    async execute(_id, params, _signal, _onUpdate, ctx) {
      ...
      return {
        content: [
          {
            type: "text",
            text: `Active VM set to ${vmId}. Persistent SSH session established. All read/bash/edit/write tools now execute on this VM.`,
          },
        ],
        details: { vmId },
      };
        ...
        content: [
          {
            type: "text",
            text: prev
              ? `Switched from VM ${prev} back to local execution. SSH session closed.`
  ...
  pi.registerTool({
    ...
    label: "bash",
    description:
      "Execute a bash command. When a Vers VM is active, executes on the VM via SSH (default timeout: 120s, configurable via --vers-ssh-timeout). Otherwise executes locally.",
    parameters: Type.Object({
      command: Type.String({ description: "Bash command to execute" }),
      timeout: Type.Optional(
        Type.Number({
          description: "Timeout in seconds. Remote default: 120s (--vers-ssh-timeout). Pass 0 for no timeout.",
        }),
      ),
    }),
  ...
  pi.registerTool({
    ...
    label: "read",
    description:
      "Read the contents of a file. When a Vers VM is active, reads from the VM via SSH. Supports offset/limit for large files.",
    parameters: Type.Object({
  ...
  pi.registerTool({
    ...
    label: "edit",
    description:
      "Edit a file by replacing exact text. When a Vers VM is active, edits the file on the VM via SSH. The oldText must match exactly.",
    parameters: Type.Object({
  ...
  pi.registerTool({
    ...
    label: "write",
    description:
      "Write content to a file. When a Vers VM is active, writes to the VM via SSH. Creates parent directories automatically.",
    parameters: Type.Object({
  ...
  pi.on("session_start", async (_event, ctx) => {
    try {
      ...
      ctx.ui.notify(`Vers connection failed: ${err instanceof Error ? err.message : String(err)}`, "warning");
    }
  });
      ...
      try {
        ...
        if (action.startsWith("use")) {
          ...
          if (command) {
            ...
            ctx.ui.notify(
              `Exit ${result.exitCode}: ${result.stdout.slice(0, 500)}`,
...
async function localEdit(
  ...
  if (content.indexOf(oldText, index + 1) !== -1)
    throw new Error(`Found multiple occurrences in ${path}. Provide more context.`);
