...
import { escapeHtml, resolveConfig } from "../shared/config.js";
...
routes.get("/_panel", async (c) => {
  ...
  if (configured) {
    try {
      const result = await slackRequest("auth.test", token!);
      statusHtml += ` — ${escapeHtml(String(result.team))} / ${escapeHtml(String(result.user))}`;
    } catch (e: any) {
...
function resolveAppToken(): string | null {
  return resolveConfig("SLACK_APP_TOKEN");
}
...
function handleSlackEvent(event: any) {
  ...
  if (!text)
    text = "The user mentioned the bot without a message. Respond with a brief greeting and what you can help with.";
...
function resolveNotificationChannel(): string | null {
  return resolveConfig("SLACK_NOTIFICATION_CHANNEL");
}
