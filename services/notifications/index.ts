...
function isMuted(): boolean {
  return resolveConfig("NOTIFICATION_MUTE") === "true";
}

function minDurationMs(): number {
  const val = resolveConfig("NOTIFICATION_MIN_DURATION_MS");
...
const EXTERNAL_TASK_TTL = 30 * 60 * 1000; // 30 min expiry for orphaned entries
const externalTaskIds = new Map<string, number>(); // taskId → timestamp

function registerExternalTask(taskId: string) {
  externalTaskIds.set(taskId, Date.now());
  // Sweep stale entries
  const now = Date.now();
  for (const [k, ts] of externalTaskIds) {
    if (now - ts > EXTERNAL_TASK_TTL) externalTaskIds.delete(k);
  }
}
...
function queueNotification(notification: Notification) {
  ...
  // Flush 5s after the FIRST buffered item (not reset on each new item)
  if (!flushTimer) {
