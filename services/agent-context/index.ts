...
async function fetchSelf(client: FleetClient) {
  ...
  const tree = await client.api<TreeVm>("GET", `/vm-tree/vms/${encodeURIComponent(vmId)}`);
  return { vmId, tree };
}

function summarizeWorkerCapacity(nodes: TreeVm[]) {
  ...
  registerTools(pi, client) {
    ...
    pi.registerTool({
      ...
      async execute() {
        ...
        try {
          const self = await fetchSelf(client);
          if (!self.tree.parentId) {
            return client.ok("This VM has no parent in the lineage tree.", { self });
          }

          const treeParent = await client.api<TreeVm>("GET", `/vm-tree/vms/${encodeURIComponent(self.tree.parentId)}`);

          const parent = {
            vmId: self.tree.parentId,
            tree: treeParent,
          };
    ...
    pi.registerTool({
      ...
      description: "Summarize this lieutenant's available worker capacity from the root reef's lineage tree.",
      parameters: Type.Object({}),
      async execute() {
        if (!client.getBaseUrl()) return client.noUrl();
        try {
          const vmId = requireVmId(client);
          const descendants = await client.api<{ descendants: TreeVm[] }>(
            "GET",
            `/vm-tree/vms/${encodeURIComponent(vmId)}/descendants`,
          );
          const summary = summarizeWorkerCapacity(descendants.descendants);
