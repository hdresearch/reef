...
function insertScheduled(input: {
  ...
  db.run(
    `INSERT INTO scheduled_checks (
      ...
      kind, message, payload, auto_cancel_on, due_at, status, created_at, updated_at
    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending', ?, ?)`,
    [
      id,
      input.ownerAgent,
...
function shouldAutoCancel(check: ScheduledCheck): string | null {
  const auto = check.autoCancelOn;
  if (!auto || !vmTreeStore) return null;

  if (auto.signalType) {
    const matched = vmTreeStore.querySignals({
      fromAgent: auto.signalFromAgent || check.targetAgent || undefined,
      signalType: auto.signalType,
      since: check.createdAt,
      limit: 1,
    });
    if (matched.length > 0) {
      return `auto-cancelled after matching signal ${auto.signalType}`;
    }
  }

  if (auto.statusIn && auto.statusIn.length > 0) {
    const target =
    ...
    if (target && auto.statusIn.includes(target.status)) {
      return `auto-cancelled after target status became ${target.status}`;
    }
  }

  if (auto.storeKey) {
    const entry = vmTreeStore.storeGet(auto.storeKey);
    if (entry) {
      if (auto.storeEquals === undefined || JSON.stringify(entry.value) === JSON.stringify(auto.storeEquals)) {
        return `auto-cancelled after store condition matched ${auto.storeKey}`;
      }
    }
  }
...
function fireScheduled(check: ScheduledCheck) {
  ...
  if (target && (target.status === "creating" || target.status === "running" || target.status === "paused")) {
    ...
    updateScheduledStatus(check.id, "fired", `delivered to ${target.name}`);
    return signal;
  }
  ...
  updateScheduledStatus(
    check.id,
    "fired",
    target ? `target ${target.name} inactive at fire time` : "no target available",
  );
  return null;
}

async function tickScheduled() {
  const due = queryScheduled({ status: "pending", dueBefore: Date.now(), limit: 50 });
...
app.post("/", async (c) => {
  ...
  const { targetAgent, targetVmId, taskId, subtreeRootVmId, kind, message, payload, autoCancelOn, delay, dueAt } =
    body as {
      targetAgent?: string;
      targetVmId?: string;
      taskId?: string;
  ...
  if (!resolvedDueAt) return c.json({ error: "delay or dueAt is required" }, 400);
...
const routeDocs: Record<string, RouteDocs> = {
  ...
  "POST /": {
    ...
    body: {
      ...
      delay: { type: "string", description: "Delay like 30s, 5m, 1h" },
      dueAt: { type: "string|number", description: "Absolute due time as ms or ISO string" },
...
const mod: ServiceModule = {
  ...
  registerTools(pi: ExtensionAPI, client: FleetClient) {
    pi.registerTool({
      ...
      description:
        "Create a durable scheduled orchestration check. Use this for future attention against fleet state instead of ad hoc reminder timers.",
      parameters: Type.Object({
        ...
        delay: Type.Optional(Type.String({ description: "Delay like 30s, 5m, 1h" })),
        dueAt: Type.Optional(Type.String({ description: "Absolute due time as an ISO timestamp" })),
      ...
      async execute(_id, params) {
        ...
        try {
          ...
          const lines = (result.checks || []).map(
            (check: any) =>
              `[${check.status}] ${check.id} ${check.kind} -> ${check.targetAgent || check.ownerAgent} @ ${new Date(check.dueAt).toLocaleTimeString()} :: ${check.message}`,
          );
